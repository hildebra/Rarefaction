...
test_that("rare works on columns and rows", {
  ...
  expect_equal(colSums(rare(data, depth= min(a), NoOfMatrices = 1, margin = 2, verbose = F)$raremat[[1]]), a)
  expect_equal(rowSums(rare(data, depth= min(b), NoOfMatrices = 1, margin = 1, verbose = F)$raremat[[1]]), b)
...
test_that("rare has the right names", {
  ...
  expect_equal(colnames(rare(data, depth=min(colSums(data)), NoOfMatrices = 1, margin = 2, verbose=F)$raremat[[1]]), cnames)
  expect_equal(rownames(rare(data, depth=min(colSums(data)), NoOfMatrices = 1, margin = 2, verbose=F)$raremat[[1]]), rnames)
  
  expect_equal(colnames(rare(data, depth=min(rowSums(data)), NoOfMatrices = 1, margin = 1, verbose=F)$raremat[[1]]), cnames)
  expect_equal(rownames(rare(data, depth=min(rowSums(data)), NoOfMatrices = 1, margin = 1, verbose=F)$raremat[[1]]), rnames)
})
...
test_that("zeros are reproduced", {
  ...
  data.r <- rare(data, depth=samplesize, NoOfMatrices = 1, margin = 2, verbose=F)$raremat[[1]]
